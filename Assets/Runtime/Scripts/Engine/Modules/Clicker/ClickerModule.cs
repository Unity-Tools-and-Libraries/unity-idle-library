...
namespace io.github.thisisnozaku.idle.framework.Engine.Modules.Clicker
{
    public class ClickerModule : IModule
    {
        ...
        {
            engine.Scripting.EvaluateString("player.points.quantity = player.points.quantity + player.points.click_income");
        }
    ...
    public static class ClickerEngineExtensionMethods
    {
        ...
        {
            return (engine.Scripting.EvaluateString(buyable.CostExpression).ToObject<BigDouble>() * new BigDouble(1.15).Pow(alreadyOwnedQuantity - 1)).Ceiling();
        }
