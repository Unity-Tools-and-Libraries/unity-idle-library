	...
	public class Table : RefIdObject, IScriptPrivateResource
	{
		...
		private Table ResolveMultipleKeys(object[] keys, out object key)
		{
			...
			for (int i = 1; i < keys.Length; ++i)
			{
				...
				if (vt == null)
					throw new ScriptRuntimeException(string.Format("Key '{0}' did not point to anything", keys[i]));

				if (vt.Type != DataType.Table)
					throw new ScriptRuntimeException(string.Format("Key '{0}' did not point to a table"));
